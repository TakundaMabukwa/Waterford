  ...
  useEffect(() => {
    if (initialRecord) {
      setFormState({
        ...
        pod_required: Boolean(initialRecord.pod_required),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const body: any = {
        ...
        pod_required: formState.pod_required === "true" || formState.pod_required === true,
        ...
        <div className="space-y-5 overflow-y-auto pr-1">
          ...
          <section className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label className="text-xs font-medium uppercase tracking-wide text-slate-600">POD Required</Label>
                <button
                  type="button"
                  onClick={() => canEditAll && updateField("pod_required", (!formState.pod_required).toString())}
                  disabled={!canEditAll}
                  className={`inline-flex h-7 w-12 items-center rounded-full transition-colors ${!canEditAll ? "opacity-50 cursor-not-allowed" : ""} ${formState.pod_required === "true" || formState.pod_required === true ? "bg-emerald-500" : "bg-slate-300"}`}
                >
                  <span className={`inline-block h-5 w-5 rounded-full bg-white shadow transition-transform ${formState.pod_required === "true" || formState.pod_required === true ? "translate-x-6" : "translate-x-1"}`} />
                </button>
                <span className="ml-2 text-xs text-slate-600">{formState.pod_required === "true" || formState.pod_required === true ? "Yes" : "No"}</span>
              </div>
              <div className="space-y-2">
